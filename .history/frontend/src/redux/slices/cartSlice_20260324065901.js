...
export const removeFromCart =createAsyncThunk("cart/removeFromCart",async({productId,guestId,userId,size,color},{rejectWithValue})=>{
  try {
    const response = await axios({method: "DELETE",url: `${import.meta.env.VITE_BACKEND_URL}/api/cart}`,data:{productId,guestId,userId,size,color}})
    return response.data
  } catch (error) {
    return rejectWithValue(error.response.data)
  }
...
export const mergeCart =createAsyncThunk("cart/export const mergeCart =createAsyncThunk("cart/removeFromCart",async({productId,guestId,userId,size,color},{rejectWithValue})=>{
