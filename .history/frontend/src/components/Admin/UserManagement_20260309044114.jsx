  ...
  return <div className="max-w-6xl mx-auto p-6">
    ...
    <div className="p-6 rounded-lg mb-6">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="bg-green-500 text-white py-2 px-4 hover:bg-green-600">Add User</button>
      </form>
    </div>
        ...
        <tbody>
          {users.map((user)=>(
            <tr key={user._id} className="border-b hover:bg-gray-50">
              ...
              <td className="p-4">
                <select name="" value={user.role} onChange={(e)=>handleRoleChange(user._id,e.target.value)} className="p-2 border rounded"></select>
              </td>
              <td className="p-4">{user.email}</td>
            </tr>
          ))}
