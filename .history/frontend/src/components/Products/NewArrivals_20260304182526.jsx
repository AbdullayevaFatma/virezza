...
// ✅ Component dışına alındı — gereksiz yeniden oluşturma engellendi
const newArrivals = [
...
const NewArrivals = () => {
  ...
  const dragRef = useRef({ isDragging: false, startX: 0, scrollLeft: 0 }); // ✅ drag state'i ref'e taşındı — gereksiz re-render engellendi

  const [canScrollRight, setCanScrollRight] = useState(true);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [isDragging, setIsDragging] = useState(false); // ✅ sadece cursor için state'te kaldı
  ...
  const handleMouseUpOrLeave = () => {
    dragRef.current.isDragging = false; // ✅ gereksiz startX hesabı kaldırıldı
    setIsDragging(false);
  };

  const scroll = (direction) => {
    const scrollAmount = direction === "left" ? -300 : 300;
    scrollRef.current?.scrollBy({ left: scrollAmount, behavior: "smooth" }); // ✅ typo düzeltildi
  };
  ...
  }, [updateScrollButtons]); // ✅ dependency eklendi
      ...
      <div className="container mx-auto text-center mb-10 relative">
        ...
        <div className="absolute right-0 -bottom-7.5 flex space-x-2">
          ...
          <button
            ...
            disabled={!canScrollRight} // ✅ eksik disabled eklendi
